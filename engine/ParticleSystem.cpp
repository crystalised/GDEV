							...
							const SParticleSetting& settings)
{
	...
	// hr = mpDevice->???, you will need to fill mpVertexBuffer
	...
	// hr=???, you will need to fill mpTexture
	if (FAILED(D3DXCreateTextureFromFile(mpDevice, inTextureFilename, &mpTexture)))
	{
		FAIL(inTextureFilename);
		return false;
	}
...
void CParticleSystem::PostDraw()
{
	...
	//Disable scaling
	mpDevice->SetRenderState(D3DRS_POINTSCALEENABLE, false);
}
...
bool CPrecipitation::Init( IDirect3DDevice9* inDevice, char* inTextureFilename,D3DXVECTOR3 dir,float variation)
{
	...
	settings.LifeTime=FLT_MAX;	// particle life is forever
...
void CExplosion::Explode(const D3DXVECTOR3& pos,float speed,int number)
{
	...
	D3DXCOLOR endCol=D3DXCOLOR(0,0,0,1);
	...
	for (int i = 0; i < number; i++)
	{
		...
		D3DXVECTOR3 vel = GetRandomDirection() * speed;
		float lifeTime = 2.0f;
		AddParticle(pos, vel, startCol, endCol, lifeTime);
	}
}
