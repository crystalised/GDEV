...
bool CTerrain::ReadHeightFile(const char* fileName )
{
	...
	if (FAILED(D3DXCreateTextureFromFile(mpDevice, fileName, &pTex)))
		return false;
...
bool CTerrain::GenerateTexture( const D3DXVECTOR3& inDirectionToLight )
{
	...
	for(int i = 0; i < theTexHeight; i++)
	{
		for(int j = 0; j < theTexWidth; j++)
		{
			...
			if (theHeight > 212.5f)
				c = D3DXCOLOR(0.831f, 0.831f, 0.831f, 1.0f);
...
{
    //TODO(CTerrain::GetPointOnGround);
...
{
    //TODO(CTerrain::IsPointAboveGround);   return false;
