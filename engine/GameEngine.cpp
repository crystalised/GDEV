...
void CGameEngine::Draw(float dt)
{
	...
	mpDevice->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB(64, 128, 255), 1.0f, 0);
...
bool CGameEngine::StartupD3D(int width,int height,bool fullscreen)
{
	...
	//mD3Dpp.SwapEffect = ??? (and quite a few other items)
	mD3Dpp.SwapEffect = D3DSWAPEFFECT_DISCARD;
	mD3Dpp.BackBufferCount = 1;
	mD3Dpp.BackBufferHeight = height;
	mD3Dpp.BackBufferWidth = width;
	mD3Dpp.hDeviceWindow = GetWindow();
	mD3Dpp.AutoDepthStencilFormat = D3DFMT_D16;
	mD3Dpp.EnableAutoDepthStencil = TRUE;

	// here we use the bool variable that was passed in
	// if the user wants a fullscreen application then 
	// we have to set mD3Dpp.Windowed to FALSE
	// and we must specify a backbufferformat
	// if this is a windowed application
	// we set mD3Dpp.Windowed to TRUE
	// and the backbufferformat can be set to D3DFMT_UNKNOWN
	if (fullscreen)
	{
		//mD3Dpp.???=...
	...
	if (SUCCEEDED(mpD3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, GetWindow(),
...
{
	// open the window
	OpenWin(GetInstance(), width, height, fullscreen);
	// setup directx
	StartupD3D(width, height, fullscreen);
	return true;
}
